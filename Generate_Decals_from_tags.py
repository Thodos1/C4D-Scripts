# (c) Maksym Vysokolov
#Select material tags to execute
#press execute
...
def xpresso_func(plane_obj, tag_m, node_master, root, x_boole, x_count, matname):
    #y_coordinates = [list(node.values())[0] for node in node_list]
    # Return the maximum y-coordinate if list is not empty; otherwise, return 0
    #return max(y_coordinates) if y_coordinates else 0
    #Y COORDINATES
    if x_boole is True:
...
def main() -> None:
    ...
    if xpresso_tag is None:
        ...
        x_boole = False
        c4d.EventAdd()
        #print("past creation point")

    node_master = xpresso_tag.GetNodeMaster()
    root = node_master.GetRoot()
    #print(node_master.IsEnabled())
    #c4d.modules.graphview.OpenDialog(0, node_master)
